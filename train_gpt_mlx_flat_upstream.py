...
# HYPERPARAMETERS
# ==============================================================================
# Default flat upstream-style run:
# - 11 flat transformer blocks at width 512
# - 8 attention heads with 4 KV heads (GQA) and 3x MLP expansion
# - SmearGate + BigramHash stem, partial RoPE, tied embeddings
...
class Hyperparameters:
    ...
    ln_scale: float = float(os.environ.get("LN_SCALE", 1.0))
    bigram_vocab_size: int = int(os.environ.get("BIGRAM_VOCAB_SIZE", 1536))
    bigram_dim: int = int(os.environ.get("BIGRAM_DIM", 128))
    ttt_steps: int = int(os.environ.get("TTT_STEPS", 0))
    ttt_lr_mul: float = float(os.environ.get("TTT_LR_MUL", 0.05))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,smear,ln_scale,bigram.scale",
    ).split(",")
    if pattern
)
...
class BigramHashEmbedding(nn.Module):
    def __init__(self, bigram_vocab_size: int, bigram_dim: int, model_dim: int):
        ...
        self.scale = mx.array([0.05, 0.025], dtype=mx.float32)

    def bigram_hash(self, tokens: mx.array) -> mx.array:
        t = tokens.astype(mx.int32)
        mod = self.bigram_vocab_size - 1
        prefix_shape = t.shape[:-1] + (1,)
        sentinel = mx.full(prefix_shape, mod, dtype=mx.int32)
        hashed = mx.bitwise_xor(36313 * t[..., 1:], 27191 * t[..., :-1]) % mod
        return mx.concatenate([sentinel, hashed], axis=-1)

    def trigram_hash(self, tokens: mx.array) -> mx.array:
        t = tokens.astype(mx.int32)
        mod = self.bigram_vocab_size - 1
        prefix_shape = t.shape[:-1] + (2,)
        sentinel = mx.full(prefix_shape, mod, dtype=mx.int32)
        hashed = mx.bitwise_xor(
            mx.bitwise_xor(14503 * t[..., 2:], 27191 * t[..., 1:-1]),
            31871 * t[..., :-2],
        ) % mod
        return mx.concatenate([sentinel, hashed], axis=-1)

    def __call__(self, token_ids: mx.array) -> mx.array:
        pair = self.embed(self.bigram_hash(token_ids))
        tri = self.embed(self.trigram_hash(token_ids))
        if self.proj is not None:
            pair = self.proj(pair)
            tri = self.proj(tri)
        scale = self.scale.astype(pair.dtype)
        return scale[0] * pair + scale[1] * tri
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        if rope_dims > self.head_dim:
            raise ValueError(f"ROPE_DIMS={rope_dims} exceeds head_dim={self.head_dim}")
        if rope_dims % 2 != 0:
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        if self.rope_dims < self.head_dim:
            q = mx.concatenate([self.rope(q[..., : self.rope_dims]), q[..., self.rope_dims :]], axis=-1)
            k = mx.concatenate([self.rope(k[..., : self.rope_dims]), k[..., self.rope_dims :]], axis=-1)
        else:
            q = self.rope(q)
            k = self.rope(k)
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: float):
        super().__init__()
        hidden = max(1, int(round(dim * mlp_mult)))
        self.fc = CastedLinear(dim, hidden)
        self.proj = CastedLinear(hidden, dim)

    def __call__(self, x: mx.array) -> mx.array:
        x = self.fc(x)
        x = mx.maximum(x, 0) + 0.5 * mx.minimum(x, 0)
...
class Block(nn.Module):
    def __init__(
        ...
        self.layer_scale = 1.0 if ln_scale <= 0 else float(ln_scale) / math.sqrt(layer_idx + 1)
    ...
    def __call__(self, x: mx.array, x0: mx.array) -> mx.array:
        ...
        if self.layer_scale != 1.0:
            attn_in = attn_in * self.layer_scale
            mlp_in = mlp_in * self.layer_scale
        attn_out = self.attn(attn_in)
        x = x + self.attn_scale.astype(x.dtype)[None, None, :] * attn_out
        x = x + self.mlp_scale.astype(x.dtype)[None, None, :] * self.mlp(mlp_in)
        return x


class GPT(nn.Module):
    # - token embedding + smear/bigram stem
    # - flat transformer trunk
    ...
    def __init__(
        ...
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, rope_dims, i, ln_scale)
...
class SplitOptimizers:
    ...
    # - learned matrices: Muon
    # - control/stem tensors: Adam
...
# ==============================================================================
# QUANTIZATION (SELECTIVE INT6 + LZMA/ZLIB)
# ==============================================================================
# - control tensors and small tensors stay in float
# - large learned matrices are stored with per-row scales
# - critical embedding/stem tensors stay in fp16 passthrough
# - only middle transformer block matrices use an int6-equivalent grid
# - fallback tensors use standard int8
...
def keep_float_array(name: str, arr: mx.array, passthrough_orig_dtypes: dict[str, str]) -> np.ndarray:
    ...
    if arr.dtype in {mx.float32, mx.bfloat16}:
        ...
        return np.ascontiguousarray(np.array(arr.astype(mx.float16), dtype=KEEP_FLOAT_STORE_DTYPE, copy=False))
    return np.ascontiguousarray(np.array(arr, copy=True))


def keep_fp16_passthrough(name: str) -> bool:
    return name in {
        "tok_emb.weight",
        "bigram.embed.weight",
        "bigram.proj.weight",
...
def quantize_float_array(arr: mx.array, clip_divisor: float = 127.0, round_to: int = 1) -> tuple[np.ndarray, np.ndarray]:
    f32 = _np_float32(arr)
    if f32.ndim == 2:
        clip_abs = np.quantile(np.abs(f32), CLIP_Q, axis=1) if f32.size else np.empty((f32.shape[0],), dtype=np.float32)
        clipped = np.clip(f32, -clip_abs[:, None], clip_abs[:, None])
        scale = np.maximum(clip_abs / clip_divisor, 1.0 / clip_divisor).astype(np.float32, copy=False)
        q = np.round(clipped / scale[:, None])
        if round_to > 1:
            q = np.round(q / round_to) * round_to
        q = np.clip(q, -127, 127).astype(np.int8, copy=False)
        return np.ascontiguousarray(q), np.ascontiguousarray(scale.astype(PER_ROW_SCALE_DTYPE, copy=False))

    clip_abs = float(np.quantile(np.abs(f32).reshape(-1), CLIP_Q)) if f32.size else 0.0
    scale = np.array(clip_abs / clip_divisor if clip_abs > 0.0 else 1.0, dtype=np.float32)
    q = np.round(np.clip(f32, -clip_abs, clip_abs) / scale)
    if round_to > 1:
        q = np.round(q / round_to) * round_to
    q = np.clip(q, -127, 127).astype(np.int8, copy=False)
...
def quantize_float_array_int6(arr: mx.array) -> tuple[np.ndarray, np.ndarray]:
    return quantize_float_array(arr, clip_divisor=127.0, round_to=4)


def quantize_state_dict_mixed(flat_state: dict[str, mx.array]) -> tuple[dict[str, object], dict[str, int]]:
    ...
    block_indices = [idx for idx in (layer_index_from_name(name) for name in flat_state) if idx is not None]
    num_layers = max(block_indices, default=-1) + 1
    edge_layers = 2 if num_layers >= 6 else 1
    for name, arr in flat_state.items():
        ...
        if not mx.issubdtype(arr.dtype, mx.floating):
            ...
            stats["quant_payload_bytes"] += int(passthrough[name].nbytes)
            continue

        if keep_fp16_passthrough(name):
            kept = keep_float_array(name, arr, passthrough_orig_dtypes)
            passthrough[name] = kept
            stats["quant_payload_bytes"] += int(kept.nbytes)
            continue
        ...
        use_int6 = (
            ...
            and edge_layers <= layer_idx < max(num_layers - edge_layers, edge_layers)
        )
        q, s = quantize_float_array_int6(arr) if use_int6 else quantize_float_array(arr)
        if s.ndim > 0:
            qmeta[name] = {"scheme": "per_row", "axis": 0}
        quantized[name] = q
        scales[name] = s
        dtypes[name] = str(arr.dtype).split(".")[-1]
        stats["quant_payload_bytes"] += int(q.nbytes + s.nbytes)
    obj: dict[str, object] = {
        "__quant_format__": "mixed_int6_int8_clean_per_row_v1",
        "quantized": quantized,
        "scales": scales,
        "dtypes": dtypes,
...
def iter_val_batches(args: Hyperparameters, val_tokens: np.ndarray, max_batches: int | None = None):
    val_batch_tokens = args.val_batch_size // args.grad_accum_steps
    if val_batch_tokens < args.train_seq_len:
        raise ValueError(
            "VAL_BATCH_SIZE must provide at least one sequence; "
...
def main() -> None:
    ...
    log(
        ...
        f"rope_dims:{args.rope_dims} seq_len:{args.train_seq_len} tie_embeddings:{args.tie_embeddings}"
    )
    log(
    ...
    log(
        ...
        f"smear_gate:{model.smear.gate.dtype}"
    )
    ...
    if args.ttt_steps > 0:
        log(f"ttt:start steps:{args.ttt_steps} lr_mul:{args.ttt_lr_mul:.4f}")
        ttt_t0 = time.perf_counter()
        for ttt_step, x_ttt, y_ttt in iter_val_batches(args, val_tokens, max_batches=args.ttt_steps):
            loss, grads = compiled_loss_and_grad(x_ttt, y_ttt)
            mx.eval(loss, grads)
            grads = clip_grad_tree(grads, args.grad_clip_norm)
            loss_value = float(loss.astype(mx.float32).item())
            opt.step(model, grads, step=args.iterations + ttt_step - 1, lr_mul=args.ttt_lr_mul)
            mx.synchronize()
            log(f"ttt:step:{ttt_step}/{args.ttt_steps} loss:{loss_value:.4f}")
        ttt_ms = 1000.0 * (time.perf_counter() - ttt_t0)
        log(f"ttt:end steps:{args.ttt_steps} eval_time:{ttt_ms:.0f}ms")
    ...
    quant_obj, quant_stats = quantize_state_dict_mixed(flat_state)
    ...
    log(
        f"serialized_model_mixed_{quant_codec}:{quant_file_bytes} bytes "
        f"(payload:{quant_stats['quant_payload_bytes']} raw_pickle:{quant_serialized_bytes} payload_ratio:{ratio:.2f}x)"
    )
    log(f"code_size:{code_bytes} bytes")
    log(f"total_submission_size:{quant_file_bytes + code_bytes} bytes")

    with quant_path.open("rb") as f:
        quant_blob_disk = f.read()
    try:
        quant_payload = lzma.decompress(quant_blob_disk)
    except Exception:
        quant_payload = zlib.decompress(quant_blob_disk)
    quant_flat = dequantize_state_dict_mixed(pickle.loads(quant_payload))
    ...
    log(f"final_mixed_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} eval_time:{q_eval_ms:.0f}ms")
    log(f"final_mixed_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
