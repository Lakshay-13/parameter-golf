...
class Hyperparameters:
    ...
    ln_scale = float(os.environ.get("LN_SCALE", 1.0))
    ...
    ttt_lr_mul = float(os.environ.get("TTT_LR_MUL", 0.05))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,smear,bigram.scale,ln_scale",
    ).split(",")
    if pattern
)
FP16_KEEP_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "tok_emb.weight,bigram.embed.weight,bigram.proj.weight",
    ).split(",")
    if pattern
)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    num_layers = int(getattr(Hyperparameters, "num_layers", 0))
    for name, tensor in state_dict.items():
        ...
        if cat == "mlp" and t.ndim >= 1:
            ...
            is_edge_layer = layer_idx is None or num_layers <= 4 or layer_idx < 2 or layer_idx >= num_layers - 2
            if ".proj." in name:
                clip = 15 if is_edge_layer else 7
                qtype = "int5" if is_edge_layer else "int4"
            else:
                clip = 15
                qtype = "int5"
...
class BigramHashEmbedding(nn.Module):
    """Hash consecutive token pairs and triplets into one learned embedding table."""
    def __init__(self, bigram_vocab_size: int, bigram_dim: int, model_dim: int):
        ...
        self.scale = nn.Parameter(torch.tensor([0.05, 0.025], dtype=torch.float32))
    ...
    def trigram_hash(self, tokens: Tensor) -> Tensor:
        ...
        out = torch.empty_like(t)
        out[..., :2] = mod
        tri = torch.bitwise_xor(
            torch.bitwise_xor(14503 * t[..., 2:], 27191 * t[..., 1:-1]),
            31871 * t[..., :-2],
        )
        out[..., 2:] = tri % mod
        return out.long()
    ...
    def forward(self, token_ids: Tensor) -> Tensor:
        ...
        if self.proj is not None:
            pair = self.proj(pair)
            tri = self.proj(tri)
        scale = self.scale.to(dtype=pair.dtype)
...
def main() -> None:
    ...
    # INT4/5 selective quantization + zstd/zlib export
    ...
    if master_process:
        with open("final_model.int45.ptz", "wb") as f:
            f.write(quant_blob)
        quant_file_bytes = os.path.getsize("final_model.int45.ptz")
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model int4/5+{_COMPRESSOR}: {quant_file_bytes} bytes")
        log0(f"Total submission size int4/5+zlib: {quant_file_bytes + code_bytes} bytes")

    if distributed:
        dist.barrier()
    with open("final_model.int45.ptz", "rb") as f:
    ...
    # Sliding window eval on int4/5-roundtripped weights
    torch.cuda.synchronize()
    ...
    torch.cuda.synchronize()
    log0(
        f"final_int45_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
        f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(f"final_int45_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
